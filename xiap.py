...
for link in range(0, len(df_link)):
    ...
    for j in range(1, 3):
        ...
        for i in range(len(html['items'])):
            ...
            views=0
            description=''
            proid=str(html['items'][0]['itemid'])+','+str(html['items'][0]['shopid'])
            purl='https://shopee.tw/api/v2/item/get?itemid=%s&shopid=%s' % (str(html['items'][0]['itemid']), str(html['items'][0]['shopid']));
            ...
            val = (title, image,images,price,category_id,shop_id,brand_id,freight_id,grounding,specs,distribution,activity,views,description,proid)
            ...
            str(html['items'][0]['itemid']), str(html['items'][0]['shopid'])),
                'accept-language': 'zh-CN,zh;q=0.9',
            ...
            str(html['items'][0]['itemid']), str(html['items'][0]['shopid']));
            preq = gethtml('https://shopee.tw/api/v2/item/get_ratings?flag=1&itemid=6259418892&limit=3&offset=0&shopid=5164899', headers)
            pjson = preq.json()
            for k in range(len(pjson['data']['ratings'])):
                ...
                images= ",".join(pjson['data']['ratings'][0]['images'])
