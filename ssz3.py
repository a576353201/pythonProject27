...
def login(username, password):
            ...
            # print driver.title
            name_input = driver.find_elements_by_class_name('a-button-text')  # 找到用户名的框框
            pass_input = driver.find_element_by_name('passwd')  # 找到输入密码的框框
            yanzheng_input = driver.find_element_by_name('piccode')  # 验证码输入框
            login_button = driver.find_element_by_id('captchacharacters')  # 找到登录按钮
            name_input.clear()
            ...
            r = session.post('https://www.ssz44444.com/api/user/balance', headers=headers1)
